...
def verify_identity(data: IdentityCheck):
    ...
    # Correct Answer -> Send Reset Token
    reset_token = "TEMP-PASS-" + hashlib.sha256(os.urandom(10)).hexdigest()[:8]
    # In real app: Email this token. For now return it.
    user["failed_attempts"] = 0 # Unlock account
    return {"message": "Identity Verified", "temp_password": reset_token}
